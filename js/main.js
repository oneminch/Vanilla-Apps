...
function deleteBookmarks() {
	...
	bookmarks.forEach(bookmark => populateBookmarks(bookmark.artclTitle,bookmark.artclSource,bookmark.artclURL, bookmark.artclID));
...
function fetchNews(tagName, keyName) {
   ...
   let query;
	...
	// Assign tag name
	if (keyName == '')
		query = !tagName ? '' : `country=us&category=${tagName}`;
	else
		query = `q=${keyName}`;
   // URL to be sent 
...
if (localStorage.length > 0) {
	...
	savedBookmarks.forEach(bookmark => populateBookmarks(bookmark.artclTitle,bookmark.artclSource,bookmark.artclURL, bookmark.artclID));
}
